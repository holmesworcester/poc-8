...
mod tests {
    ...
    fn purge_event_storage_removes_local_retention_rows_but_keeps_labels() {
        ...
        event_schema::insert_event(&store, &record, EventStatus::Ready).expect("insert event");
        event_schema::insert_blocked_event_missing_dep(&store, &missing_dep_id, &target_id)
        ...
        assert_eq!(
            event_schema::blocked_events_by_missing_dep(&store, &missing_dep_id)
                .expect("forward deps"),
            Vec::<EventId>::new()
        );
        assert!(
            !event_schema::blocked_event_has_missing_deps(&store, &target_id)
                .expect("reverse deps")
        );
        assert_eq!(
