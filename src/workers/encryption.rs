...
fn retire_deleted_event_leaf<R: EventRegistry>(
    ...
    eprintln!(
        "[retire] start width={} start={} is_root={}",
        current.range_width, current.range_start, current.is_root
    );
    while current.range_width > 1 {
        let half = current.range_width / 2;
        let mid = current.range_start + half;
        ...
        eprintln!("[retire] split parent_start={parent_start} parent_width={parent_width} descend_side={descend_side} descend_start={descend_start} half={half}");
