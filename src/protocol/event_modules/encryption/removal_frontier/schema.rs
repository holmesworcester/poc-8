...
//!
//! Rows are keyed by `workspace_id || removal_frontier_id` because key secrets,
//! wraps, and encrypted content all name the frontier by event id. The value
//! stores the admin authority and bounded removal refs. Those refs are a
//! compact removal boundary whose dependency closure matters; they are not a
