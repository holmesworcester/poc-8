...
//! an admin grant in the same workspace. Non-empty frontier refs are rejected
//! until the removal fact type lands, so this projector does not accept generic
//! unrelated event ids as removal refs.
...
pub fn project(event: &EventWithContext<'_>) -> Result<ProjectionOutput, String> {
    ...
    if !frontier.removal_event_ids.is_empty() {
        return Err("non-empty removal frontier refs are not supported yet".to_string());
    }
...
mod tests {
    ...
    fn projects_frontier_row_from_admin_endpoint() {
        ...
        let record = frontier_record([1; 32], signer_id, signer_private_key, admin_id);
        let event = event_with_context(&record, signer_id, signer_record, admin_id, admin_record);
