...
//!
//! Derivation is deterministic from the source secret, range, frontier, and
//! optional tombstone id. The command returns one local event and does not write
//! tombstone rows directly; projection owns row replacement semantics after the
//! common worker has loaded dependencies.
