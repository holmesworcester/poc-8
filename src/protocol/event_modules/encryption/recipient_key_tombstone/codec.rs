...
pub fn decode_signed(bytes: &[u8]) -> Result<SignedRecipientKeyTombstoneEnvelope, String> {
    let mut reader = Reader::new(bytes, "signed recipient key tombstone envelope");
    let tag = reader.u8()?;
    if tag != TYPE_SIGNED_RECIPIENT_KEY_TOMBSTONE {
        return Err("expected signed recipient key tombstone envelope".to_string());
    }
    let signer_endpoint_shared_id = reader.id()?;
    let signer_public_key = reader.id()?;
    let payload = reader.bytes(RECIPIENT_KEY_TOMBSTONE_WIRE_SIZE)?;
