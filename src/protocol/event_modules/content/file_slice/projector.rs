...
pub fn project(event: &EventWithContext<'_>) -> Result<ProjectionOutput, String> {
    ...
    let verified = codec::verify_slice_proof(
...
mod tests {
    ...
    fn projects_one_slice_row_with_bao_verified_plaintext() {
        ...
        let (root_hash, outboard) = codec::outboard_for_plaintext(&plaintext).expect("outboard");
    ...
    fn rejects_slice_whose_proof_does_not_match_descriptor_root_hash() {
        ...
        let (_real_root, outboard) = codec::outboard_for_plaintext(&plaintext).expect("outboard");
        let descriptor = descriptor_record(
            workspace_id,
            file_id,
            plaintext.len() as u64,
