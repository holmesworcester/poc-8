...
/// Fixed ciphertext budget per full slice.
///
/// This ports the poc-7 slot-capacity fix to encrypted file slices. BAO proofs
/// grow sharply when slice starts drift off the proof chunk boundary; if we
/// allowed 256 KiB of plaintext and then appended the AEAD tag, every slice
/// after the first would start 16 bytes later than the previous proof chunk.
/// Keeping full-slice ciphertext at 256 KiB preserves bounded BAO proof sizes.
...
pub const FILE_SLICE_PROOF_BYTES: usize = FILE_SLICE_CIPHERTEXT_BYTES + BAO_ENCODING_BUDGET_BYTES;
