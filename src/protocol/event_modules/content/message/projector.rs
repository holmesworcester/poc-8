...
//!
//! This file also owns `admit_check_received`, the receive-side gate the
//! content domain dispatches into before message bytes enter storage. It
//! sits next to `project()` because both functions enforce per-event
//! validation rules: `project()` decides what rows to write for an
//! admitted event, and `admit_check_received` decides whether the event
//! is admissible at all (drop a re-delivery of a tombstoned message, or
//! tombstone-on-arrival a stamped-expiry message whose minute is past).
