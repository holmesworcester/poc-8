...
fn encode_file_value(
    ...
    let filename =
        codec::encode_text_slot(&event.filename, FILE_NAME_BYTES, "file filename")?;
    let mime_type =
        codec::encode_text_slot(&event.mime_type, FILE_MIME_BYTES, "file mime type")?;
    let mut out = Writer::with_capacity(8 + 32 + 32 + 32 + 32 + 8 + 4 + 4 + 32 + FILE_NAME_BYTES + FILE_MIME_BYTES);
