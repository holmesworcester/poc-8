...
impl FileSummary {
    pub fn lines(&self) -> Vec<String> {
        ...
        out.push(format!("   file_id: {}", message::cli::hex_id(self.file_id)));
        out.push(format!(
            "   message: {}",
...
fn run_files_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let limit = match args.get(1) {
        Some(value) => value.parse::<usize>().map_err(|_| FILES_USAGE.to_string())?,
        None => 0,
    };
...
pub fn list_summaries(
    ...
    for (idx, row) in rows.into_iter().enumerate() {
        ...
        let deleted = message::cli::is_deleted_by_author(
            store,
            &row.message_id,
            &row.author_user_id,
        )?;
