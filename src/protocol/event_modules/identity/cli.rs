...
fn run_grant_admin_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let workspace_id = decode_hex_32(args.get(0).expect("length checked"), GRANT_ADMIN_USAGE)?;
    let target_user_id = decode_hex_32(args.get(1).expect("length checked"), GRANT_ADMIN_USAGE)?;
    let local = ensure_local_endpoint(context)?;
...
fn run_users_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let workspace_id = decode_hex_32(args.get(0).expect("length checked"), USERS_USAGE)?;
    let mut lines = Vec::new();
...
fn run_peers_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let workspace_id = decode_hex_32(args.get(0).expect("length checked"), PEERS_USAGE)?;
    let mut lines = Vec::new();
    for (key, value) in context
        .store
...
fn connect_invite(
    ...
    connect_invite_with_initial_records(context, link, Vec::new())
}

fn connect_invite_with_initial_records(
    context: &mut Context,
    link: &str,
...
fn proposed_records(events: &[worker::ProposedEvent]) -> Vec<EventRecord> {
